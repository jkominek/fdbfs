...
struct AttemptState_rename : public AttemptStateT<ActionT> {
...
class Inflight_rename
    : public InflightWithAttemptT<AttemptState_rename<ActionT>,
...
template <typename ActionT>
bool Inflight_rename<ActionT>::write_success_oplog_result() {
  OpLogResultOK result;
  return write_oplog_result(result);
}

template <typename ActionT>
ActionT Inflight_rename<ActionT>::oplog_recovery(const OpLogRecord &record) {
...
template <typename ActionT> ActionT Inflight_rename<ActionT>::check() {
  ...
  if (oldparent_inode.inode() == newparent_inode.inode()) {
    ...
    if (auto it = update_directory_times(
            transaction.get(), oldparent_inode,
            (oldparent_nlink_delta + newparent_nlink_delta) == 0
                ? DirectoryUpdateKind::ContentsDeferred
                : DirectoryUpdateKind::ContentsPersist);
        !it.has_value()) {
    ...
    if (auto it = update_directory_times(
            transaction.get(), oldparent_inode,
            oldparent_nlink_delta == 0 ? DirectoryUpdateKind::ContentsDeferred
                                       : DirectoryUpdateKind::ContentsPersist);
        !it.has_value()) {
      return ActionT::FDBError(it.error());
    }
    if (auto it = update_directory_times(
            transaction.get(), newparent_inode,
            newparent_nlink_delta == 0 ? DirectoryUpdateKind::ContentsDeferred
                                       : DirectoryUpdateKind::ContentsPersist);
        !it.has_value()) {
      return ActionT::FDBError(it.error());
    }
