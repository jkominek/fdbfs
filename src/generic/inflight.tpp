...
template <typename ActionT> ActionT Inflight_markusedT<ActionT>::check_inode() {
  ...
  if (!present) {
    (void)decrement_lookup_count(entry_inode.inode(), 1);
    return ActionT::Abort(EIO);
  }

  auto use_key = pack_inode_use_key(entry_inode.inode());
