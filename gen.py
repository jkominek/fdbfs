...
def printable(s):
    return "".join(
        [
            bytes([c]).decode("ascii") if c in printable_bytes else ("\\x%02x" % (c,))
            for c in s
        ]
    )
...
def prefix_end(p):
    ...
    for i in range(len(b) - 1, -1, -1):
        if b[i] != 0xFF:
            b[i] += 1
            return bytes(b[: i + 1])
    raise ValueError("prefix has no lexicographic successor")
...
print("writemode on")
print("begin")
print("clearrange {} {}".format(printable(prefix), printable(prefix_end(prefix))))
...
if False:
    # Sample fixture entries kept around for occasional manual testing.
    hello_inode = 42
    ...
    print_set(
        xattr_node_key(hello_inode, b"greeting"), hello_xattr_node.SerializeToString()
    )
    print_set(xattr_data_key(hello_inode, b"greeting"), b"bonjour!")
    ...
    print_set(
        block_key(world_inode, 0) + b"z\x01\x01",
        zstd.ZSTD_compress(world_inode_data, 22),
    )
