...
void create_file_with_contents(
    const fs::path &p, std::string_view s,
    std::source_location loc = std::source_location::current()) {
  INFO("create_file_with_contents caller=" << loc.file_name() << ":"
                                           << loc.line());
  INFO("path=" << p << " size=" << s.size());
...
TEST_CASE("rename type and emptiness checks", "[integration][rename]") {
  scenario([&](FdbfsEnv &env) {
    ...
    FDBFS_REQUIRE_OK(::mkdir(dir1.c_str(), 0755));
    FDBFS_REQUIRE_OK(::mkdir(dir2.c_str(), 0755));
